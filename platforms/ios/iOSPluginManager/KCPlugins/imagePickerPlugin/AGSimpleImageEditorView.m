...
- (void)setRatioViewBorderColor:(UIColor *)theRatioViewBorderColor
{
    ...
    {
        ratioViewBorderColor = theRatioViewBorderColor;
...
- (UIImage *)output
{
    ...
    UIImage *croppedImage = [self croppedImage:nil];
...
- (void)dealloc
{
    ...
    didChangeCropRectBlock = nil;
    didChangeRotationBlock = nil;
...
- (void)initialize
{
    ...
    imageRationView = [[UIView alloc] initWithFrame:self.frame];
    imageRationView.userInteractionEnabled = YES;
    imageRationView.autoresizesSubviews = YES;
    ...
    [imageRationView addSubview:imageView];
    [self addSubview:imageRationView];
    
    // Ratio Controls View
    //    ratioControlsView = [[UIView alloc] initWithFrame:imageView.frame];
    //    ratioControlsView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 320, 480)];
    ...
    ////    [ratioView addGestureRecognizer:rotationGestureRecognizer];
...
- (id)initWithAsset:(ALAsset *)theAsset image:(UIImage *)theImage andFrame:(CGRect)frame
{
    ...
    if (self)
    {
        ...
        self.asset = theAsset;
        self.image = theImage;
        [_photoScrollView addSubview:imageRationView];
    }
...
- (id)initWithAsset:(ALAsset *)theAsset andImage:(UIImage *)theImage
{
    return [self initWithAsset:theAsset image:theImage andFrame:CGRectMake(0, 0, 320.f, 480.f)];
}
...
{
    CGPoint translation = [recognizer translationInView:self];//获得偏移的坐标
...
- (UIImage *)croppedImage:(UIImage *)imageToCrop
{
    ...
    //                                       );
    //
    ////    actualCropRect = CGRectMake(0, 0, self.frame.size.width, self.frame.size.width);
    //
    //    UIImage *outputImage = nil;
    //
...
- (UIView *)viewForZoomingInScrollView:(UIScrollView *)scrollView {
    return imageRationView;
}

-(void)scrollViewWillBeginZooming:(UIScrollView *)scrollView withView:(UIView *)view
{
    CGAffineTransform transform = imageView.transform;
    //    self.beforeTranform = transform;
    NSLog(@"image view transform befor Zooming: a:%f, b:%f, c:%f, d:%f, tx:%f, ty:%f \n\n",transform.a,transform.b,transform.c,transform.d,transform.tx,transform.ty);
}
...
{
    CGAffineTransform transform = imageView.transform;
    //    imageView.transform = self.beforeTranform;
    NSLog(@"image view transform after Zooming: a:%f, b:%f, c:%f, d:%f, tx:%f, ty:%f \n\n",transform.a,transform.b,transform.c,transform.d,transform.tx,transform.ty);
...
- (void)scrollViewDidZoom:(UIScrollView *)scrollView{
    ...
    //    CGAffineTransform transform = imageView.transform;
    //    if (self.beforeTranform.a >= 0 && self.beforeTranform.b > 0 && self.beforeTranform.c > 0 && self.beforeTranform.d > 0 ) {
    //        imageView.transform = self.beforeTranform;
    //    }
    ...
    [imageRationView setCenter:CGPointMake(xcenter, ycenter)];
}
